...
function daowuTrack(player, cards) {
    ...
    if (!Array.isArray(player.storage.tia2_daowu_tracked)) player.storage.tia2_daowu_tracked = [];
    for (const card of cards) {
        if (card && card.cardid && !player.storage.tia2_daowu_tracked.includes(card.cardid)) {
            player.storage.tia2_daowu_tracked.push(card.cardid);
        }
    }
}
...
const skills = {
    ...
    tia2_qiangli: {
        ...
        filter(event, player) {
            ...
            if (event.card.storage && event.card.storage.tia2_rongguang) return false;
        ...
        mod: {
            cardUsable(card, player, num) {
                ...
                if (card.storage && card.storage.tia2_qiangli_recast && (card.name === "sha" || card.name === "jiu")) return Infinity;
                if (get.itemtype(card) !== "card" || get.position(card) !== "h") return;
                if (card.name === "sha" || card.name === "jiu") return Infinity;
            },
            cardEnabled(card, player) {
                if (!player.isPhaseUsing()) return;
                if (card.storage && card.storage.tia2_qiangli_recast && (card.name === "sha" || card.name === "jiu")) return true;
                if (get.itemtype(card) !== "card" || get.position(card) !== "h") return;
                if (card.name === "sha" || card.name === "jiu") return true;
            },
            selectTarget(card, player, range) {
                if (!player.isPhaseUsing()) return;
                if (card.storage && card.storage.tia2_qiangli_recast && (card.name === "sha" || card.name === "jiu")) {
                    range[0] = -1;
                    range[1] = -1;
                    return;
                }
            ...
            targetInRange(card, player, target) {
                if (!player.isPhaseUsing()) return;
                if (card.storage && card.storage.tia2_qiangli_recast && card.name === "sha") return true;
                if (get.itemtype(card) !== "card" || get.position(card) !== "h") return;
                if (card.name === "sha") return true;
            },
        },
        group: ["tia2_qiangli_recast", "tia2_qiangli_draw"],
        subSkill: {
            recast: {
                enable: "phaseUse",
                filter(event, player) {
                ...
                viewAs(cards, player) {
                    ...
                    return {
                        ...
                        isCard: true,
                        storage: { tia2_qiangli_recast: true },
                    };
                },
            ...
            draw: {
                ...
                filter(event, player) {
                    ...
                    if (event.card.storage && event.card.storage.tia2_rongguang) return false;
                    if (!(event.card.storage && event.card.storage.tia2_qiangli_recast) && !(Array.isArray(event.cards) && event.cards.length > 0 && event.cards.every(card => ["sha", "jiu"].includes(get.name(card, player))))) return false;
    ...
    tia2_rongguang: {
        ...
        async precontent(event, trigger, player) {
            ...
            card.storage.tia2_rongguang = true;
            card.storage.tia2_rongguang_state = {
                round: 1,
                currentPoint: ammo.number,
            };
            if (ammo.source === "daowu") card.nature = "fire";
            player.logSkill("tia2_rongguang");
        ...
        group: ["tia2_rongguang_track", "tia2_rongguang_extra"],
    },

    tia2_rongguang_track: {
        charlotte: true,
        trigger: { player: "useCard1" },
        forced: true,
        popup: false,
        filter(event, player) {
            return !!(event.card && event.card.storage && event.card.storage.tia2_rongguang);
        },
        async content(event, trigger, player) {
            const state = trigger.card.storage.tia2_rongguang_state;
            if (!state) return;
            if (!trigger.storage) trigger.storage = {};
            trigger.storage.tia2_rongguang_state = state;
    ...
    tia2_rongguang_extra: {
        ...
        filter(event, player) {
            ...
            return !!(useCard && useCard.card && useCard.card.storage && useCard.card.storage.tia2_rongguang && getAmmo(player).length);
        },
        async content(event, trigger, player) {
            const useCard = trigger.getParent("useCard") || trigger.parent;
            if (!useCard || !useCard.storage || !useCard.storage.tia2_rongguang_state) return;
            const state = useCard.storage.tia2_rongguang_state;
            ...
            const choice = await player.chooseBool()
                .set("prompt", get.prompt("tia2_rongguang"))
    ...
    tia2_daowu: {
        ...
        chooseButton: {
            ...
            backup(links, player) {
                ...
                return {
                    viewAs: { name, isCard: true, storage: { tia2_daowu: true } },
                    filterCard(card, player) {
                    ...
                    async precontent(event, trigger, player) {
                        if (event.result.card) {
                            ...
                            event.result.card.storage.tia2_daowu = true;
                        }
    ...
    tia2_daowu_collect: {
        ...
        popup: false,
        filter(event, player) {
            const tracked = player.storage.tia2_daowu_tracked;
        ...
        async content(event, trigger, player) {
            ...
            if (!cards.length) return;
            for (const card of cards) {
                const index = player.storage.tia2_daowu_tracked.indexOf(card.cardid);
                if (index >= 0) player.storage.tia2_daowu_tracked.splice(index, 1);
                if (getAmmo(player).length >= MAX_AMMO) continue;
                const result = await player.chooseBool(get.prompt("tia2_daowu"), "是否将" + get.translation(card) + "明置为一发弹药？")
                    .set("choice", true)
                    .forResult();
                if (!result.bool) continue;
                await game.cardsGotoSpecial(card);
                addAmmoFromCard(player, card, true, "daowu");
                player.logSkill("tia2_daowu");
