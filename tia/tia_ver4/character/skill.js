...
function lostHandCards(event, player) {
    ...
    return lose && Array.isArray(lose.hs) ? lose.hs : [];
}
...
function daowuTrack(player, cards) {
    ...
    if (!Array.isArray(player.storage.tia4_daowu_tracked)) player.storage.tia4_daowu_tracked = [];
    for (const card of cards) {
        if (card && card.cardid && !player.storage.tia4_daowu_tracked.includes(card.cardid)) {
            player.storage.tia4_daowu_tracked.push(card.cardid);
        }
    }
}
...
const skills = {
    ...
    tia4_qiangli: {
        ...
        mod: {
            cardname(card, player) {
                if (isEntityShaOrJiu(card)) return "tia4_danyi";
            },
            cardEnabled(card, player) {
                if (isEntityShaOrJiu(card)) return true;
            },
    ...
    tia4_rongguang: {
        ...
        async precontent(event, trigger, player) {
            ...
            card.storage.tia4_rongguang = true;
            card.storage.tia4_rongguang_state = {
                round: 1,
                currentPoint: ammo.number,
            };
            if (ammo.source === "daowu") card.nature = "fire";
            player.logSkill("tia4_rongguang");
        ...
        group: ["tia4_rongguang_track", "tia4_rongguang_extra"],
    },

    tia4_rongguang_track: {
        charlotte: true,
        trigger: { player: "useCard1" },
        forced: true,
        popup: false,
        filter(event, player) {
            return !!(event.card && event.card.storage && event.card.storage.tia4_rongguang);
        },
        async content(event, trigger, player) {
            const state = trigger.card.storage.tia4_rongguang_state;
            if (!state) return;
            if (!trigger.storage) trigger.storage = {};
            trigger.storage.tia4_rongguang_state = state;
    ...
    tia4_rongguang_extra: {
        ...
        filter(event, player) {
            ...
            return !!(useCard && useCard.card && useCard.card.storage && useCard.card.storage.tia4_rongguang && getAmmo(player).length);
        },
        async content(event, trigger, player) {
            const useCard = trigger.getParent("useCard") || trigger.parent;
            if (!useCard || !useCard.storage || !useCard.storage.tia4_rongguang_state) return;
            const state = useCard.storage.tia4_rongguang_state;
            ...
            const choice = await player.chooseBool()
                .set("prompt", get.prompt("tia4_rongguang"))
    ...
    tia4_daowu: {
        ...
        chooseButton: {
            ...
            backup(links, player) {
                ...
                return {
                    viewAs: { name, isCard: true, storage: { tia4_daowu: true } },
                    ...
                    async precontent(event, trigger, player) {
                        if (event.result.card) {
                            ...
                            event.result.card.storage.tia4_daowu = true;
                        }
        ...
        group: ["tia4_daowu_darkload", "tia4_daowu_collect"],
    },
    ...
    tia4_daowu_darkload: {
        ...
        filter(event, player) {
            if (_status.currentPhase === player) return false;
            return lostHandCards(event, player).length > 0;
        },
        async content(event, trigger, player) {
            player.logSkill("tia4_daowu");
            await darkLoad(player, "daowu_dark");
        },
    },
    ...
    tia4_daowu_collect: {
        ...
        popup: false,
        filter(event, player) {
            const tracked = player.storage.tia4_daowu_tracked;
        ...
        async content(event, trigger, player) {
            ...
            for (const card of cards) {
                const index = player.storage.tia4_daowu_tracked.indexOf(card.cardid);
                if (index >= 0) player.storage.tia4_daowu_tracked.splice(index, 1);
                const result = await player.chooseBool(get.prompt("tia4_daowu"), "是否将" + get.translation(card) + "明置为一发弹药？")
                    .set("choice", true)
                    .forResult();
