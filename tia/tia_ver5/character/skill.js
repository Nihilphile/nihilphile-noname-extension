...
const skills = {
    ...
    tia5_qiangli: {
        ...
        mod: {
            cardname(card, player) {
                if (_status.currentPhase === player && isEntityShaOrJiu(card)) return "tia5_danyi";
            },
    ...
    tia5_rongguang: {
        ...
        async precontent(event, trigger, player) {
            ...
            card.storage.tia5_rongguang = true;
            card.storage.tia5_rongguang_state = {
                round: 1,
                currentPoint: ammo.number,
                id: rongguangId,
            };
            if (ammo.source === "daowu") card.nature = "fire";
            player.logSkill("tia5_rongguang");
        ...
        group: ["tia5_rongguang_track", "tia5_rongguang_extra"],
    },

    tia5_rongguang_track: {
        charlotte: true,
        trigger: { player: "useCard1" },
        forced: true,
        popup: false,
        filter(event, player) {
            return !!(event.card && event.card.storage && event.card.storage.tia5_rongguang);
        },
        async content(event, trigger, player) {
            const state = trigger.card.storage.tia5_rongguang_state;
            if (!state) return;
            if (!trigger.storage) trigger.storage = {};
            trigger.storage.tia5_rongguang_state = state;
            ...
            trigger._tia5_rongguang_id = state.id;
    ...
    tia5_rongguang_extra: {
        ...
        filter(event, player) {
            ...
            if (!useCard || !useCard._tia5_rongguang_id) return false;
            if (!getAmmo(player).length) return false;
        ...
        async content(event, trigger, player) {
            ...
            if (!useCard || !useCard._tia5_rongguang_id) return;
            if (!useCard.storage || !useCard.storage.tia5_rongguang_state) return;
            const state = useCard.storage.tia5_rongguang_state;
            const targets = useCard.targets || [];
            ...
            const choice = await player.chooseBool()
                .set("prompt", get.prompt("tia5_rongguang"))
    ...
    tia5_daowu: {
        ...
        chooseButton: {
            ...
            backup(links, player) {
                ...
                return {
                    viewAs: { name, isCard: true, storage: { tia5_daowu: true } },
                    ...
                    async precontent(event, trigger, player) {
                        if (event.result.card) {
                            ...
                            event.result.card.storage.tia5_daowu = true;
                        }
                    },
                };
            },
    ...
    tia5_daowu_collect: {
        ...
        filter(event, player) {
            ...
            if (event.card.storage && event.card.storage.tia5_rongguang) return false;
        ...
        async content(event, trigger, player) {
            ...
            for (const entityCard of cards) {
                ...
                const result = await player.chooseBool(get.prompt("tia5_daowu"), "是否将" + get.translation(entityCard) + "明置为一发弹药？")
                    .set("choice", true)
                    .forResult();
                if (!result.bool) continue;
                await game.cardsGotoSpecial(entityCard);
                addAmmoFromCard(player, entityCard, true, "daowu");
                player.logSkill("tia5_daowu");
